...
public class ListEx1 {
	...
	public static void main(String[] args) {
		...
		/* list3는 List인터페이스를 구현한 구현 클래스들의 객체 중 아무나 올 수 있다.
		 ...
		 * - 리스트에서는 무조건 true //중복O
		 * - set에서는 없으면 true, 있으면 false //중복X
		 */
		List<String> list5 = new ArrayList<String>();
		list5.add("abc");
		list5.add("def");
		
		/* contains(객체): Collection, 컬력센에 객체가 있는지 없는지를 알려줌
		 * - 이 때, Objects.equals를 이용하여 동작
		 * - 컬렉션에 있는 타입과 객체의 타입이 다르면 false를 리턴
		 * - 같으면 객체.equals()를 호출해서 결과를 리턴
		 * - 해당 클래스의 equals가 어떻게 오버라이딩 되어 있느냐에 따라 결과가 달라짐
		 */
		System.out.println(list5.contains("abc"));
		
		/* indexOf(객체) : List, 리스트에 객체가 몇 번지에 있는지 알려주고 없으면 -1.
		 *  - 동장 방식은 contains와 같음. 리턴값만 번지.
		 */
		...
		/* iterator() : Collention, 컬렉션을 이용하여 반복자를 생성해서 리턴 
		 * -> 반복자를 이용하여 반복문으로 활용
		 * */
		java.util.Iterator<String> it = list5.iterator();
		while(it.hasNext()) {
			String tmp = it.next();
		...
		/* sort() : List, 람다식을 이용해서 정렬 기준을 정해주면 정해진 정렬기준대로 리스트 정렬
		...
		/* Collections.sort() : 리스트의 타입이 comparable 인터페이스를 구현한 
		 * 구현 인터페이스인 경우 해당 기준을 이용하여 정렬
		 * 
		 */
