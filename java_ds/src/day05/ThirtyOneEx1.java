...
public class ThirtyOneEx1 {
	...
	public static void main(String[] args) {
	    ...
	    //현재 부른 숫자를 저장할 변수 선언 후 0으로 초기화
	     int count = 0;
	     int min = 1, max = 3;	     
	     //반복문(현재 숫자가 31이 아닐 때 까지)
	     while(count < 31) {
	    	 //컴퓨터가 랜덤으로 1~3사이의 숫자를 생성
	    	 int com = (int)(Math.random() * (max-min+1)+min);
	    	 int end = count + com;
	    	 
	    	 //현재 숫자부터 랜덤으로 생성된 숫자를 합했을 때 
	    	 //31이상이 되면 31까지 출력하고 컴퓨터가 졌다고 출력 후
	    	 //반복문 종료
	    	 ...
	    	 //샤용자가 1~3사이의 숫자를 입력
	    	 System.out.println("1~3사이의 숫자를 입력 하세요 :");
	    	 user = scan.nextInt();
	    	 
	    	 
	    	 //현재 숫자부터 입력한 숫자를 합했을 때 31이상 되면
	    	 //31까지 출력하고 사용자가 졌다고 출력 후
	    	 //반복문 종료
	    	 end = count + user;
	    	 end = end >= 31 ? 31 : end;
